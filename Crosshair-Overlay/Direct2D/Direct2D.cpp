...
void RenderText(std::string str, float fontSize, float x, float y, float r, float g, float b, float a)
{
	...
	if (SUCCEEDED(res))
	{
		...
		solid_brush->SetColor(D2D1::ColorF(r / 255, g / 255, b / 255, a / 255));
...
{
	RenderText(str, fontSize, x, y, 0, 0, 0, a);
	RenderText(str, fontSize, x, y + 2, r, g, b, a);
}
...
{
	RenderLine(x - 7, y, x, y + 4, 2, r, g, b, a);
	RenderLine(x, y + 4, x + 5, y - 8, 2, r, g, b, a);
}

void RenderCross(float x, float y, float r, float g, float b, float a)
{
	RenderLine(x - 5, y - 5, x + 5, y + 5, 2, r, g, b, a);
	RenderLine(x + 5, y - 5, x - 5, y + 5, 2, r, g, b, a);
}
...
void InitializeWindow(HWND tWindow) {
	...
	overlayWindow = CreateWindowEx(WS_EX_LAYERED | WS_EX_TRANSPARENT | WS_EX_TOPMOST,
		wc.lpszClassName, "Overlay", WS_POPUP, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT,
...
DWORD WINAPI OverlayThread(LPVOID lpParam)
{
	...
	InitializeWindow(enumWindow);
	for (;;) {
		mainLoop();
	}
}

void InitializeOverlay(DirectOverlayCallback callback, HWND _targetWindow) {
